...
app.post('/api/payment', async (req, res) => {
    ...
    try {
        ...
        const sign_string = crypto
            .createHash('md5')
            .update(`${CLICK_SECRET_KEY}${amount}${order_id}${CLICK_LIVE_API_KEY}`)
            .digest('hex');
        ...
        if (response.data.error) {
            console.error(`Click API xatoligi: ${response.data.error_note}`);
            throw new Error(`Click API xatoligi: ${response.data.error_note}`);
        }

        const paymentUrl = response.data.payment_url;
        console.log('Payment URL:', paymentUrl); // Yangi log qo'shildi
